...
public class ProjectModel {
    public String getNextProjectId() throws SQLException {
        ResultSet rst = CrudUtil.execute("select Project_ID from Project order by Project_ID desc limit 1");
    ...
    public ProjectDto findByProjectId(String projectId) throws SQLException {
        ResultSet rst = CrudUtil.execute("SELECT * FROM Project WHERE Project_ID=?", projectId);
        if (rst.next()) {
    ...
    public String findClientNameById(String clientId) throws SQLException {
        ResultSet rst = CrudUtil.execute("SELECT Name FROM Client WHERE Client_ID=?", clientId);
        if (rst.next()) {
    ...
    public ArrayList<String> getAllProjectIdsByClient(String clientId) throws SQLException {
        ResultSet rst = CrudUtil.execute("SELECT Project_ID FROM Project WHERE Client_ID=?", clientId);
        ArrayList<String> projectIds = new ArrayList<>();
        while (rst.next()) {
            projectIds.add(rst.getString(1));
        }
...
//                projectDto.getClientId()
    ...
    public List<ProjectDto> getAllProjects() throws SQLException {
        ResultSet resultSet = CrudUtil.execute("SELECT * FROM Project order by Project_ID asc");
